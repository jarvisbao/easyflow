 ...
 */
import translate from './i18n/customTranslate'
...
var nodeExtension = function() {
  function check(node, reporter) {
    if (is$c(node, 'bpmn:StartEvent')) {
      if (!node.eventDefinitions) { // 空开始节点
        const noneEvent = getExtension(node, 'enfo:NoneEventDefinition');
        if (!noneEvent || !noneEvent.assigns) {
          reporter.report(node.id, '请完成开始节点人员配置');
        }
      }
    }
    ...
    if (is$c(node, 'bpmn:StartEvent') || is$c(node, 'bpmn:IntermediateThrowEvent') || is$c(node, 'bpmn:IntermediateCatchEvent')) {
      ...
      if (hasEventDefinition(node, 'bpmn:SignalEventDefinition')) {
        if (node.$type === 'bpmn:IntermediateThrowEvent') {
          if (!node.eventDefinitions[0].signalRef || !node.eventDefinitions[0].$attrs.producer) {
            reporter.report(node.id, '请完成信号事件的节点相关配置');
          }
        } else {
          if (!node.eventDefinitions[0].signalRef || !node.eventDefinitions[0].$attrs.consumer) {
            reporter.report(node.id, '请完成信号事件的节点相关配置');
          }
        }
      }
...
var idRequired = function() {
  function check(node, reporter) {
    if (isAny$d(node, [
      ...
      if (id.length === 0) {
        reporter.report(node.id, translate('元素缺少ID'));
      }
    }
  }
  return { check };
...
cache['bpmnlint/node-extension'] = nodeExtension;
cache['bpmnlint/id-required'] = idRequired;
